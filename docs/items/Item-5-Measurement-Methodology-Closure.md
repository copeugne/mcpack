...
The corrected harness waits for `Profiler stopped & save complete!` before flushing and stopping. The accepted pilot receipt records exact commands, timestamps, runtime identities, seed, raw artifacts, deterministic processed output and hashes. Its input-world, configuration-tree, stopped-world snapshot, Spark overlay, Spark artifact, and complete 137-JAR runtime identity are also recorded in the raw lifecycle receipt and cross-checked by the validator. New runs preflight every runtime JAR against the retained and acquisition manifests. They also resolve and execute the exact `--java-home/bin/java`, reject a missing or non-Temurin-21 runtime before launch, and record the observed version. The replacement preserved log identifies Java `21.0.12.1` by Eclipse Adoptium; the validator requires that observation and binds receipt seed/player labels and Minecraft/NeoForge/Java versions to the raw CSV and compressed runtime log. The validator requires both accepted and rejected paths whenever pilot receipts are supplied.
