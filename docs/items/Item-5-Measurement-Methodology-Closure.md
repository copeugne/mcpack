...
The corrected harness waits for `Profiler stopped & save complete!` before flushing and stopping. The accepted pilot receipt records exact commands, timestamps, runtime identities, seed, raw artifacts, deterministic processed output and hashes. Its input-world, configuration-tree, stopped-world snapshot, Spark overlay, Spark artifact, and complete 137-JAR runtime identity are also recorded in the raw lifecycle receipt and cross-checked by the validator. New runs preflight every runtime JAR against the retained and acquisition manifests. They also resolve and execute the exact `--java-home/bin/java`, require the `Temurin-21.0.12.1+1` build marker before launch, and record the observed build. The receipt binds the Java installation to the platform manifest's pinned archive SHA-256 `ce79869e1307ed8ee1e2baa86a412b1eb5b75d10a01006d788a6f968bcfaee94`. The replacement preserved log identifies Java `21.0.12.1` by Eclipse Adoptium; the validator requires that observation and binds receipt seed/player labels and Minecraft/NeoForge/Java identities to the raw CSV and compressed runtime log. The validator requires both accepted and rejected paths whenever pilot receipts are supplied.

## Exit gate

**PASS.** The corrected clean full-stack pilot ran under pinned Temurin `21.0.12.1+1-LTS`, profiled for 30 seconds measured from Spark’s asynchronous start confirmation, saved a 236,557-byte local profile, completed `save-all flush`, and stopped with exit code 0. Both accepted and rejected evidence paths validate with all artifact hashes. After review strengthened the Java identity contract, the accepted pilot was rerun from a fresh seed-42 materialization; the earlier Java-25 pilot was superseded rather than relabeled. This operational pilot proves collection, preservation, processing and rejection handling; its short, dirty-worktree startup sample is explicitly not a performance baseline or tuning claim. Item 6 is eligible.
