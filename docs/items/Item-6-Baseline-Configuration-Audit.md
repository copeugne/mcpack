...
The YUNG structure inventory includes the Cristel Lib placement and toggle files for Better Desert Temples, Better Dungeons, and Better Nether Fortresses. The placement evidence records every spacing and separation value. The toggle evidence records every enabled structure leaf. Sparse Structures remains a separate global factor that may interact with these enabled placements.
