...
The active branch is `codex/item-8-completion`, based on verified `origin/main` merge `b0e4fc0`. The predeclared evidence scope and current remaining work are in `evidence/item-8/README.md`, under "Current delivery work at 934dbcb". Runtime and packaged-data collection are delivered, with 887 roots assigned to 421 provisional groups. Provider coverage, canonical reconciliation, required attributes and final review/delivery remain open; no canonical count is accepted yet.
...
| 8 | Structure-family inventory | `IN PROGRESS` | Working inventory delivered through `934dbcb`; provider coverage, canonical family reconciliation and required attributes remain incomplete. Current work order and definition of done are in `evidence/item-8/README.md`. Registry raw custody, publication and downloaded restore delivered through `63cb1f8`, under `evidence/item-8/raw-custody/`. Full local gate, clean final Codex review and verified main merge remain required. |
