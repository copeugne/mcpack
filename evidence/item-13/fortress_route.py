...
def check_route():
    for x, y, z in route:
        ...
        if state["Name"] == "minecraft:nether_brick_stairs":
            assert (
                state["Properties"]["half"] == "bottom"
                and state["Properties"]["shape"] == "straight"
            )
            # The centered .6-wide actor overlaps the upper supporting half by .3.
            # Collision avoidance still uses the conservative full-cell box.
        b = shape(x, y - 1, z)
    ...
    for (a, b), crouch in zip(pairwise(route), crouches, strict=True):
        ...
        high = max(a[1], b[1])
        horizontal = abs(b[0] - a[0]) + abs(b[2] - a[2])
        assert horizontal in (1, 2)
        if horizontal == 2:
            assert a[1] == b[1] and not crouch
            apex = high + 1.3  # Conservative full-jump envelope for a one-cell slot.
            clear(
                [
                    min(a[0], b[0]) + 0.2,
                    high,
...
def ordinary():  # noqa: PLR0915
    ...
    for u, cu, j, n, a, s, k, v, duty in (
        ...
        print(
            json.dumps({"no_enemy_seconds": base, "two_blaze_seconds": base + 2 * n + 5.2 / duty})
        )
