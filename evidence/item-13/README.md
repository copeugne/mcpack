...
sample in 20 families. Sixteen satisfy their declared local sampling scope;
Small Dungeon, Circle Ruin, Underground Temple and Repurposed Temple retain exact gaps. The other 172
...
Available evidence needing integration: 24 of the 172 have full baseline start
candidates,17 more have adequate Item8 references, and Starlight Voyager has its
verified Item7 pair. These 42 input-ready families still need sample/material
...
The existing scope implies at least 524 remaining case completions, divided into
4 compact, 244 fixed and 276 modular. This is a conservative assessment floor,
not 524 new experiments or a finished selection matrix. Missing component outcomes
...
are `Nc >= 4`, `Nf >= 244`, `Nm >= 276`, because uncovered material/components can
...
| New fixed analysis/integration | Desert Mimic:2,180s through one full result. [Nether group](fixed-blocks/repurposed-nether-temples-report.md#group-overrun-and-retained-crimson-geometry):150s selection/read and5,505s analysis for one full plus one partial result; cap exceeded. Crimson then required592s bounded integration. Warped required565s whole-case analysis/integration including construction and its distinct source/reward task; wasteland457s including overlap access and quality integration; soul649s including fractional support and complete-task correction | General unit cost `Af` UNKNOWN from these differing incomplete/complete batches; at least244 further case assessments |
...
524-case floor establishes the work scale; the minute-scale extractor timings do
...
variants. The [warped complete case](fixed-blocks/repurposed-nether-temples-report.md#warped-complete-local-quality-result)
adds its constructed upper link, distinct reward allocation and non-hostile source.
The [wasteland result](fixed-blocks/repurposed-nether-temples-report.md#wasteland-complete-local-quality-result)
adds its actual fortress-overlap constraints and complete neutral-source task.
The [soul result](fixed-blocks/repurposed-nether-temples-report.md#soul-complete-local-quality-result)
adds actual fractional support, sand slowdown and the complete skeleton task.
This is five satisfied roots of seven, not whole-family completion.

Next: match the ocean/taiga alternatives against existing supplemental raw and
prior Item8/Item7 references before declaring any new experiment. The five Nether
reads and full local results are complete within their declared sample scope.
Do not repeat them. Exact remaining sample/terrain gaps must determine the next
bounded work. Total ETA and compact/modular unit costs remain UNKNOWN; full
acceptance and the original failed group budget remain unchanged.
