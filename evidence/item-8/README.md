...
for coverage, not accepted as an authored-structure family. Only that row and
the decision input identity change. The focused test and scoped static checks
passed. Reproduce:

```sh
uv run -m tools.build_item8_inventory --output evidence/raw/item8/inventory-cloud-6ab0446.json
cmp evidence/item-8/inventory.json evidence/raw/item8/inventory-cloud-6ab0446.json
...
templates. It changes only the monument family and decision input identity.
Effective processors, placement, geometry and discoverability remain open.
Six affected tests and scoped static checks passed. Reproduce:

```sh
uv run -m tools.build_item8_inventory --output evidence/raw/item8/inventory-monument-62f005a.json
cmp evidence/item-8/inventory.json evidence/raw/item8/inventory-monument-62f005a.json
uv run pytest -q tests/item8/test_monument_components.py tests/item8/test_inventory_sources.py
```
...
settings and open dependencies are explicit. Thirty focused tests and scoped
checks passed. Reproduce with:

```sh
uv run -m tools.build_item8_inventory --output evidence/raw/item8/inventory-fallen-log-ec8a1e9.json
cmp evidence/item-8/inventory.json evidence/raw/item8/inventory-fallen-log-ec8a1e9.json
```
...
reaches the captured static hook and selects the custom generator under frozen
configuration. Nominal bounds are derived, not observed occupied blocks.
Thirty focused tests and scoped builder checks passed. Reproduce with:

```sh
uv run -m tools.build_item8_inventory --output evidence/raw/item8/inventory-platform-caller-6032002.json
cmp evidence/item-8/inventory.json evidence/raw/item8/inventory-platform-caller-6032002.json
```
...
matching row from the catalog and checks runtime registration and captured
End-only possible-biome membership. This does not establish the complete effective
feature list after code modifications or every lifecycle caller. Thirty focused
tests passed. Initial test typing errors were corrected; scoped Ruff/Basedpyright
and the affected test then passed. Reproduce with:

```sh
uv run -m tools.build_item8_inventory --output evidence/raw/item8/inventory-end-biomes-9025a02.json
cmp evidence/item-8/inventory.json evidence/raw/item8/inventory-end-biomes-9025a02.json
```
