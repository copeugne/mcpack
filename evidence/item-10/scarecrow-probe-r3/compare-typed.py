...
if bundle_path.exists() or summary_path.exists():
    raise ValueError("Typed comparison outputs must be absent")
selections = {
...
for label, (dimension, bounds) in selections.items():
    coordinates = {(x, z) for x in range(bounds[0], bounds[1] + 1)
    ...
    for member in ("probe", "control"):
        ...
        if (data["generation_encoding"] != "typed-nbt-v2"
                or data["dimension"] != dimension or data["bounds_chunks"] != bounds
                or data["full_chunks"] != len(coordinates)
                or len(vectors) != len(coordinates) or set(indexed) != coordinates):
            raise ValueError(f"Wrong encoding or incomplete declared frame: {name}")
        inputs[name] = data
        input_hashes[name] = hashlib.sha256(payload).hexdigest()
        members.append(indexed)
    mismatches = sorted(position for position in coordinates if members[0][position] != members[1][position])
    comparisons.append({"selection": label, "dimension": dimension, "bounds_chunks": bounds,
                        "chunks": len(coordinates), "mismatch_count": len(mismatches),
...
summary = {"generation_encoding": "typed-nbt-v2", "input_sha256": input_hashes,
           ...
           "comparisons": [{key: value for key, value in row.items() if key != "mismatch_coordinates"}
