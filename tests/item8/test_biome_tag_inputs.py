...
def test_pool_tags_reuse_merge_rules_and_preserve_both_contributors() -> None:
    first = source("lithostitched.jar", {"values": ["minecraft:zombie"]})
    second = source("regions_unexplored.jar", {"values": ["regions_unexplored:ashen"]})
    for row in (first, second):
        row["path"] = "data/example/tags/worldgen/template_pool/melee.json"
    merged = tag_inputs([first, second], kind="tags/worldgen/template_pool")
    melee = cast("dict[str, JsonValue]", merged["example:melee"])
    assert melee["values"] == ["minecraft:zombie", "regions_unexplored:ashen"]
    assert len(cast("list[JsonValue]", melee["sources"])) == 2
    assert melee["unresolved"] == []
    assert tag_inputs([first, second]) == {}
...
def test_additive_sources_and_vanilla_replacement_do_not_depend_on_catalog_order() -> None:
    ...
    result = tag_inputs([mod, vanilla])
    assert result == tag_inputs([vanilla, mod])
    ...
    row = cast("dict[str, JsonValue]", tag_inputs([vanilla, additive])["example:test"])
...
def test_condition_removal_and_optional_pack_are_not_silently_merged() -> None:
    for field in ("neoforge:conditions", "remove"):
        ...
        result = cast("dict[str, JsonValue]", tag_inputs([row])["example:test"])
