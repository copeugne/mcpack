...
def test_yungs_bridges_full_archive_has_only_known_feature_paths() -> None:
    source = next(s for s in retained_sources(Path.cwd()) if s.name.startswith("YungsBridges-"))
    assert hashlib.sha256(source.path.read_bytes()).hexdigest() == source.sha256
    classes: set[str] = set()
    directories = (
        "yungs-bridge-generation",
        "yungs-bridge-processors",
        "yungs-bridges-module-loader",
        "yungs-bridges-module-default",
        "yungs-bridges-scope-entries",
    )
    with ZipFile(source.path) as archive:
        ...
        service = "com.yungnickyoung.minecraft.yungsbridges.services."
        metadata = {
            "META-INF/MANIFEST.MF",
            "META-INF/neoforge.mods.toml",
            "pack.mcmeta",
            "yungsbridges.mixins.json",
            "LICENSE_YungsBridges",
        ...
        groups = {
            prefix: {n for n in remaining if n.startswith(f"data/yungsbridges/{prefix}/")}
            for prefix in (
                "structure",
        ...
        assert tuple(len(v) for v in groups.values()) == (14, 23, 1, 5, 1, 1)
        assert all(n.endswith(".nbt") for n in groups["structure"])
        assert all(n.endswith(".json") for n in remaining - groups["structure"])
        mixins = cast("dict[str, object]", json.loads(archive.read("yungsbridges.mixins.json")))
        assert mixins["mixins"] == ["SuppressLogMixin"]
