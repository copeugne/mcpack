...
from mcpack_evidence.item8_sources import packaged_sources
...
def test_preserves_competing_optional_and_nested_resources(tmp_path: Path) -> None:
    ...
    result = packaged_sources((first, second))
    ...
    assert packaged_sources((first, second)) == result
...
def test_comment_parsing_and_invalid_json_are_explicit(tmp_path: Path) -> None:
    ...
    rows = cast("list[dict[str, JsonValue]]", packaged_sources((item,))["resources"])
    assert rows[0]["parser"] == "json-with-line-comments-removed"
    invalid = source(tmp_path, "bad.jar", {path: b"{invalid}"})
    failures = cast("list[dict[str, JsonValue]]", packaged_sources((invalid,))["resources"])
