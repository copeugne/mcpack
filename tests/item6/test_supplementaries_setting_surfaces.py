...
SYSTEM = "Supplementaries"
FILES = [
    "config/cristellib/supplementaries/structure_placement_config.json5",
    "config/cristellib/supplementaries/structure_toggle_config.json5",
]


def test_supplementaries_surfaces_reproduce_all_8_source_leaves() -> None:
    ...
    assert [len(surface["leaves"]) for surface in surfaces] == [6, 2]


def test_supplementaries_files_are_system_owned_and_audited() -> None:
...
def test_supplementaries_surface_mutation_is_rejected(tmp_path: Path) -> None:
