...
SYSTEM = "Explorations"
FILES = [
    "config/cristellib/explorations/structure_placement_config.json5",
    "config/cristellib/explorations/structure_toggle_config.json5",
]


def test_explorations_surfaces_reproduce_all_40_source_leaves() -> None:
    ...
    assert [len(surface["leaves"]) for surface in surfaces] == [30, 10]


def test_explorations_files_are_system_owned_and_audited() -> None:
...
def test_explorations_surface_mutation_is_rejected(tmp_path: Path) -> None:
