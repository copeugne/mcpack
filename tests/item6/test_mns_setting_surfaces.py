...
FILES = [
    "config/cristellib/mns/structure_placement_config.json5",
    "config/cristellib/mns/structure_toggle_config.json5",
]


def test_mns_surfaces_reproduce_all_178_source_leaves() -> None:
    # Given: both committed MNS configuration surfaces.
    surfaces = [surface for surface in AUDIT_DATA["setting_surfaces"] if surface["file"] in FILES]

    # When/Then: generator output exactly covers 126 placement and 52 toggle leaves.
    assert surfaces == [
        build_setting_surface(SYSTEM, relative, FROZEN / relative) for relative in FILES
    ]
    assert [len(surface["leaves"]) for surface in surfaces] == [126, 52]


def test_mns_files_are_system_owned_and_audited() -> None:
    # Given: the two MNS files and the committed audit classifications.
...
def test_mns_surface_mutation_is_rejected(tmp_path: Path) -> None:
    # Given: one MNS leaf key is changed without changing the frozen source.
    audit = deepcopy(AUDIT_DATA)
    surface = next(surface for surface in audit["setting_surfaces"] if surface["file"] == FILES[0])
    surface["leaves"][0]["key"] = "mns_common.wrong"

    # When/Then: exact source validation rejects the false MNS claim.
