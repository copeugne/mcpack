...
FILES = [
    "config/cristellib/repurposed_structures/structure_placement_config.json5",
    "config/cristellib/repurposed_structures/structure_toggle_config.json5",
]


def test_repurposed_surfaces_reproduce_all_222_source_leaves() -> None:
    # Given: both committed Repurposed Structures configuration surfaces.
    surfaces = [surface for surface in AUDIT_DATA["setting_surfaces"] if surface["file"] in FILES]

    # When/Then: generator output exactly covers 115 placement and 107 toggle leaves.
    assert surfaces == [
        build_setting_surface(SYSTEM, relative, FROZEN / relative) for relative in FILES
    ]
    assert [len(surface["leaves"]) for surface in surfaces] == [115, 107]


def test_repurposed_files_are_system_owned_and_audited() -> None:
    # Given: the two grouped files and the committed audit classifications.
...
def test_repurposed_surface_mutation_is_rejected(tmp_path: Path) -> None:
    # Given: one Repurposed Structures leaf value changes without changing the source.
    audit = deepcopy(AUDIT_DATA)
    surface = next(surface for surface in audit["setting_surfaces"] if surface["file"] == FILES[0])
    surface["leaves"][0]["effective_value"] = False
