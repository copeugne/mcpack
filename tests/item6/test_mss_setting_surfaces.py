...
FILES = [
    "config/cristellib/mss/structure_placement_config.json5",
    "config/cristellib/mss/structure_toggle_config.json5",
]


def test_mss_surfaces_reproduce_all_140_source_leaves() -> None:
    # Given: both committed MSS configuration surfaces.
    surfaces = [surface for surface in AUDIT_DATA["setting_surfaces"] if surface["file"] in FILES]

    # When/Then: generator output exactly covers 105 placement and 35 toggle leaves.
    assert surfaces == [
        build_setting_surface(SYSTEM, relative, FROZEN / relative) for relative in FILES
    ]
    assert [len(surface["leaves"]) for surface in surfaces] == [105, 35]


def test_mss_files_are_system_owned_and_audited() -> None:
    # Given: the two MSS files and the committed audit classifications.
...
def test_mss_surface_mutation_is_rejected(tmp_path: Path) -> None:
    # Given: one MSS leaf prefix is changed without changing the frozen source.
    audit = deepcopy(AUDIT_DATA)
    surface = next(surface for surface in audit["setting_surfaces"] if surface["file"] == FILES[0])
    surface["leaves"][0]["prefix"] = "wrong"

    # When/Then: exact source validation rejects the false MSS claim.
