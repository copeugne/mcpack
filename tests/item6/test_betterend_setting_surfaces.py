...
SYSTEM = "BetterEnd"
FILES = [
    "config/cristellib/betterend/structure_placement_config.json5",
    "config/cristellib/betterend/structure_toggle_config.json5",
]


def test_betterend_surfaces_reproduce_all_50_source_leaves() -> None:
    ...
    assert [len(surface["leaves"]) for surface in surfaces] == [36, 14]


def test_betterend_files_are_system_owned_and_audited() -> None:
...
def test_betterend_surface_mutation_is_rejected(tmp_path: Path) -> None:
