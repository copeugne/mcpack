...
SYSTEM = "Explorify"
FILES = [
    "config/cristellib/explorify/structure_placement_config.json5",
    "config/cristellib/explorify/structure_toggle_config.json5",
]


def test_explorify_surfaces_reproduce_all_76_source_leaves() -> None:
    ...
    assert [len(surface["leaves"]) for surface in surfaces] == [53, 23]


def test_explorify_files_are_system_owned_and_audited() -> None:
...
def test_explorify_surface_mutation_is_rejected(tmp_path: Path) -> None:
