...
SYSTEM = "Creating Space"
FILES = [
    "config/cristellib/creatingspace/structure_placement_config.json5",
    "config/cristellib/creatingspace/structure_toggle_config.json5",
]


def test_creating_space_surfaces_reproduce_all_16_source_leaves() -> None:
    ...
    assert [len(surface["leaves"]) for surface in surfaces] == [12, 4]


def test_creating_space_files_are_system_owned_and_audited() -> None:
...
def test_creating_space_surface_mutation_is_rejected(tmp_path: Path) -> None:
