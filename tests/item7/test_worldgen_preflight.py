...
from tests.item7.runtime_support import FROZEN, AcquisitionDocument, runtime_request


def test_preflight_materializes_exact_frozen_runtime(tmp_path: Path) -> None:
    ...
    assert (
        receipt.chunky_sha256 == "d72f235cf1f56f2c374f52c00bdda5034524b28142305a84cfc123a3f92ad274"
    )
...
def test_preflight_rejects_existing_target_and_wrong_retained_count(tmp_path: Path) -> None:
    existing = runtime_request(tmp_path / "existing")
    ...
    wrong_count = runtime_request(tmp_path / "wrong-count")
    ...
    wrong_overlay = runtime_request(tmp_path / "wrong-overlay")
...
def test_java_preflight_requires_exact_temurin_build(tmp_path: Path) -> None:
    request = runtime_request(tmp_path)
