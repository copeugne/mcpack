...
- Exact-SHA reviews at `97262a21b0b76c253f57e32b8665e48d0a63f822` and `8c7e7b8bb5db79d826b78cab5a678605a8b5fc23` returned `REJECTED`. The second review passed every non-security lane but found six valid filesystem and repository-binding defects. Commits `fdd99d9` and `c625d6e` fix those defects with focused RED-to-GREEN coverage. Both rejected reviews are preserved under `evidence/item-7/review/`. A fresh final-SHA review remains mandatory.
...
The Item 7 completion command returns `PASS` and records 123 exact artifacts in `evidence/item-7/completion.json`. The focused Item 7 suite passes with 162 tests in the working tree. The prior 155-test candidate also passed in a clean `git archive` export. A fresh clean-export gate remains required for the corrected candidate. The repository-bound r2 raw-release verifier redownloaded all four assets and matched every committed size and SHA-256.
